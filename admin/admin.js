...
function showToast(message, type = "loading", duration = 3000) {
  ...
  if (type === "loading") icon = '<div class="toast-loader"></div>';
    ...
    icon =
      '<div class="toast-icon"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg></div>';
...
onAuthStateChanged(auth, async (user) => {
  ...
  try {
    ...
    initializeSession(user.uid).catch((err) =>
...
function showStatsSkeleton() {
  ...
  stats.forEach((el) => {
    el.innerHTML =
      '<div class="skeleton skeleton-text" style="width: 60px; height: 1.8rem; margin: 0.5rem auto;"></div>';
  });
}
    ...
    (snapshot) => {
      ...
      document.querySelectorAll(".edit-btn").forEach((btn) => {
        btn.addEventListener("click", () =>
          openEditModal(btn.dataset.id, snapshot),
        );
      });
...
function setupRealtimeNotifications() {
  ...
  if (markReadBtn) {
    markReadBtn.onclick = async () => {
      const unreadDocs = await getDocs(
        query(collection(db, "messages"), where("status", "!=", "read")),
      );
  ...
  const handleSnapshot = (snapshot) => {
    const unreadCount =
      snapshot.size ||
      snapshot.docs?.filter((d) => d.data().status !== "read").length ||
      0;
      ...
      if (unreadCount === 0) {
        ...
        docs
          ...
          .forEach((docSnap) => {
            ...
            item.querySelector(".notif-content").onclick = () =>
            ...
            if (
              ...
              if (navigator.serviceWorker && navigator.serviceWorker.ready) {
                navigator.serviceWorker.ready.then((reg) =>
                  reg.showNotification("New Message from " + msg.name, options),
                );
              }
            }

            // Mark as notified in the database immediately so other tabs/sessions don't re-show it
            updateDoc(doc(db, "messages", id), { notified: true }).catch(
              (err) => console.error("Error marking as notified:", err),
            );
          }
        }
      });
  ...
  onSnapshot(
    ...
    (error) => {
      console.warn(
        "Complex notification query failed (missing index?). Using fallback.",
      );
...
function renderMessages(snapshot) {
  ...
  snapshot.forEach((docSnap) => {
    ...
    tr.innerHTML = `
            ...
            <td data-label="Message" class="msg-preview desktop-only">${msg.message}</td>
            <td data-label="Actions">
                <div class="action-dropdown">
                    <button class="action-btn ellipsis-btn" onclick="event.stopPropagation(); window.toggleDropdown('${id}')">
                    ...
                    <div id="dropdown-${id}" class="dropdown-content">
                        ...
                        <a href="https://wa.me/${msg.phone?.replace(/\+/g, "")}" target="_blank" onclick="event.stopPropagation()" style="display: flex; align-items: center; gap: 0.5rem;">
                            <img src="../images/whatsapp.png" alt="" width="14" height="14">
                            WhatsApp
                        </a>
...
window.viewMessage = async (id) => {
  try {
    ...
    document.getElementById("msg-detail-phone").textContent =
      msg.phone || "N/A";
    document.getElementById("msg-detail-date").textContent = msg.createdAt
      ? msg.createdAt.toDate
    ...
    const cleanPhone = msg.phone ? msg.phone.replace(/\+/g, "") : "";
    document.getElementById("msg-call-btn").href = `tel:${msg.phone}`;
    document.getElementById("msg-whatsapp-btn").href =
...
if (nextMsgBtn)
  nextMsgBtn.addEventListener("click", () => {
    loadMessages("next");
    if (prevMsgBtn) prevMsgBtn.disabled = false;
  });
if (prevMsgBtn)
  prevMsgBtn.addEventListener("click", () => loadMessages("prev"));
...
function loadAnalytics() {
  ...
  onSnapshot(vQuery, (snapshot) => {
    ...
    if (activeUsersCount)
      activeUsersCount.textContent = Math.floor(Math.random() * 3) + 1;
    ...
    if (countryList)
      renderAnalyticsList(countryList, countries, totalUniqueVisits);
  });
}
...
if (settingsForm) {
  settingsForm.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      console.error(error);
      msgEl.textContent = "Error updating password. Re-login may be required.";
      msgEl.style.color = "var(--error-color)";
      msgEl.classList.remove("hidden");
    } finally {
      btn.disabled = false;
...
async function loadVisitors() {
  ...
  onSnapshot(
    query(collection(db, "visitors"), orderBy("lastVisit", "desc"), limit(100)),
    (snapshot) => {
      table.innerHTML = "";
      snapshot.forEach((docSnap) => {
        const data = docSnap.data();
        const date = data.lastVisit
          ? data.lastVisit.toDate
            ? data.lastVisit.toDate().toLocaleString()
...
function renderVisitsGraph(snapshot, canvasId = "visits-chart") {
  ...
  snapshot.forEach((docSnap) => {
    const data = docSnap.data();
    if (data.lastVisit) {
      const date = data.lastVisit.toDate
        ? data.lastVisit.toDate().toLocaleDateString()
...
const sessionId =
  localStorage.getItem("admin_session_id") || crypto.randomUUID();
...
function setupSessionNotifications() {
  ...
  const q = query(
    ...
    where("loginAt", ">", dashboardLoadTime),
...
async function loadSessions() {
  ...
  onSnapshot(
    ...
    (snapshot) => {
      ...
      if (snapshot.empty) {
        sessionsList.innerHTML =
          '<p style="text-align: center; padding: 2rem; color: var(--text-light);">No active sessions found.</p>';
        return;
      }
    ...
    (error) => {
      ...
      sessionsList.innerHTML =
        '<p style="text-align: center; padding: 2rem; color: var(--error-color);">Error loading sessions. Check console.</p>';
    },
  );
}
...
async function loadSiteSettings() {
  ...
  try {
    ...
    if (siteDoc.exists()) {
      ...
      document.getElementById("social-whatsapp-msg").value =
        data.whatsappMsg || "";
    }
  } catch (error) {
  ...
  siteForm.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      msgEl.textContent =
        "Settings saved successfully! Changes will reflect on your portfolio shortly.";
